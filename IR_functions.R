...
IRs<- function(gbfiles, file="IR.jpg"){
...
IRs2<- function(gbfiles, file="IR.jpg"){
  ...
  par(mai=c(0.5, 1+max(chr.count(unlist(spnames)))/20, 0.7, 0.5))# c(bottom, left, top, right) 
...
JunctRadiusFinderDinp<- function(gene.cordinates, IRinp, J.pos, radius, silence=TRUE){#function to find the genes near the junction cordinate based on the output of the 
  ...
  #for its first, second, third, fourth, and fifth values respectively gene.cordinate function
  if(J.pos==1){
    J<- IRinp[1]
  }
  else if(J.pos==2){
  ...
  fluidRow(column(width = 1, ""), column(width = 11, offset = 1, 
                                         tabsetPanel(
                                           tabPanel(strong("Home"), br(), 
                                                    column( width=6, h4(tags$em("Welcome to IRscope...")), br(), tags$p(strong("IRscope"),"is a tool for  visualizing the genes on the boundaries of the junction sites of the chloroplast genome. The input files can be uploaded using the 'Files upload' tab. After the submission is done, the program will start searching the", tags$em("Inverted Repeat"), "regions. Note that the species seleceted will lead to a consensus radius for each junction site for the genes to be plotted. This indicates that if the input files are not representing evolutionary closely related species, the program may lead to unsatisfactory result. Finlally note that the input files can be either manual annotations, GeneBank accession numbers, or related GB files of the chloropalst genomes.")
                                                    )),
                                           tabPanel("Files upload", br(), fluidRow(column(width = 11, br(), p("There are two situations that you can use IRscope. First: with providing the GB files or the accession number of the related species- under 'GB File' tab, or second: with providing the annotation files in a simple tabular format besides the genome sequences of the targeted species and possibly their coresponding inverted repeat regions boundries coordinates - under 'Manual Files' tab. Further instruction for each case is given specifically under relevant tab."), br(),  p("You need to provide information related to at least 2 species for the analysis to proceed. After you have successfully provided your inputs, press the 'Submit' botton. This will innactivate its following box. Be patient till your are communicated through that box on how to proceed for downloading your output. Also note that the computations may take up to five minutes. Please be patient."))),
                                                    br(), br(),br(),
                                                    tabsetPanel(
                                                      tabPanel("GB File", br(), br(), p("In this section you can upload either your GeneBank files or provide their coresponding accession numbers if aplicable. For example you can test the program with these accession numbers:"), tags$em("NC_007898, NC_008096"), br(), br(),
                                                               fluidRow(column(width = 2, textInput(inputId = "acc1", label = "Accession No.", placeholder = "1st Accession ...")),
                                                                        column(width = 3,  fileInput(inputId = "data1", label = "GeneBank File", placeholder = "or GB file")), column(width = 5, br(), verbatimTextOutput("sp1", placeholder = TRUE)), column(width = 2, br())
                                                               ),
                                                               fluidRow(column(width = 2, textInput(inputId = "acc2", label = "", placeholder = "2nd Accession ...")),
                                                                        column(width = 3,  fileInput(inputId = "data2", label = "", placeholder = "or GB file")), column(width = 5, br(), verbatimTextOutput("sp2", placeholder = TRUE)), column(width = 2, br())
                                                               ),
                                                               fluidRow(column(width = 2, textInput(inputId = "acc3", label = "", placeholder = "3rd Accession ...")),
                                                                        column(width = 3,  fileInput(inputId = "data3", label = "", placeholder = "or GB file")), column(width = 5, br(), verbatimTextOutput("sp3", placeholder = TRUE)), column(width = 2, br())
                                                               ),
                                                               fluidRow(column(width = 2, textInput(inputId = "acc4", label = "", placeholder = "4th Accession ...")),
                                                                        column(width = 3,  fileInput(inputId = "data4", label = "", placeholder = "or GB file")), column(width = 5, br(), verbatimTextOutput("sp4", placeholder = TRUE)), column(width = 2, br())
                                                               ),
                                                               fluidRow(column(width = 2, textInput(inputId = "acc5", label = "", placeholder = "5th Accession ...")),
                                                                        column(width = 3,  fileInput(inputId = "data5", label = "", placeholder = "or GB file")), column(width = 5, br(), verbatimTextOutput("sp5", placeholder = TRUE)), column(width = 2, br())
                                                               ),
                                                               fluidRow(column(width = 2, textInput(inputId = "acc6", label = "", placeholder = "6th Accession ...")),
                                                                        column(width = 3,  fileInput(inputId = "data6", label = "", placeholder = "or GB file")), column(width = 5, br(), verbatimTextOutput("sp6", placeholder = TRUE)), column(width = 2, br())
                                                               ),
                                                               fluidRow(column(width = 2, textInput(inputId = "acc7", label = "", placeholder = "7th Accession ...")),
                                                                        column(width = 3,  fileInput(inputId = "data7", label = "", placeholder = "or GB file")), column(width = 5, br(), verbatimTextOutput("sp7", placeholder = TRUE)), column(width = 2, br())
                                                               ),
                                                               fluidRow(column(width = 2, textInput(inputId = "acc8", label = "", placeholder = "8th Accession ...")),
                                                                        column(width = 3,  fileInput(inputId = "data8", label = "", placeholder = "or GB file")), column(width = 5, br(), verbatimTextOutput("sp8", placeholder = TRUE)), column(width = 2, br())
                                                               ),
                                                               fluidRow(column(width = 2, textInput(inputId = "acc9", label = "", placeholder = "9th Accession ...")),
                                                                        column(width = 3,  fileInput(inputId = "data9", label = "", placeholder = "or GB file")), column(width = 5, br(), verbatimTextOutput("sp9", placeholder = TRUE)), column(width = 2, br())
                                                               ),
                                                               fluidRow(column(width = 2, textInput(inputId = "acc10", label = "", placeholder = "10th Accession ...")),
                                                                        column(width = 3,  fileInput(inputId = "data10", label = "", placeholder = "or GB file")), column(width = 5, br(), verbatimTextOutput("sp10", placeholder = TRUE)), column(width = 2, br())
                                                               ),
                                                      ...
                                                      tabPanel("Manual Files", br(), p("In case your GB files are of low quality or otherwise not available and still you would like to obtain the plot, you can provide your annotations as a simple tabular format (primary Dogma output) beside their genomes in fasta format and proceed to obtain the plot. In case you also have the values for the junction sites coordinates, you can provide them on the 'IR info' section as JLB, JSB, JSA, and JLA respectively. This decreases the processing time dramatically and would be useful in cases where for example the IR regions are not identical. A minimal example of a annotation input can be like", downloadLink('downloadDogma', 'this'), "file."),
                                           ...
                                           tabPanel("FAQ", br(), column(width = 1, ""), br(), column(width = 6, h4(strong("Q:"), tags$em(strong("Why the gene names are not displayed properly?"))), p(strong("A:"),"Check your input file(s), IRscope is based on the gene names specified in them."), br(),
                                                                                                     h4(strong("Q:"), tags$em(strong("Why some of the genes are extended to the big portion of the tracks?"))), p(strong("A:"),"This is due to the bad annotation implemented in the input file(s). Often this is related to the genes with introns or trans-spliced genes like rps12. Check and fix your input files."), br(),
                                                                                                     h4(strong("Q:"), tags$em(strong("I have a well annotated genome but why I am not getting the plot?"))), p(strong("A:"),"IRscope can handle the redundant base pairs but if the sequence is of a very poor quality the program may fail in detecting the inverted regions and hence not produce any output."), br(),
                                                                                                     h4(strong("Q:"), tags$em(strong("I see genes overplotting on each other, how can I fix it?"))), p(strong("A:"),"This is mostly because at least one of your species differs significantly from the rest so that it creates too largre radius for finding the genes in the vicinity of the junction site and consequently causes the populated genes in this respective areas. Try reducing the species sampling to a smaller group of more closely related species."), br(),
                                                                                                     h4(strong("Q:"), tags$em(strong("Can I modify and run the codes on my own computer?"))), p(strong("A:"),"Yes, please download the file in the 'About' section, run the whole script in your R session. This will set up a pseudo web interface on your computer which resemle and works exactly like our online instance. You can further tune the functions to your likings." ), br(),
                                                                                                     h4(strong("Q:"), tags$em(strong("My analysis runs slow. Can I enhance its speed?"))), p(strong("A:"),"Yes, after downloading the file in the 'About' section, alter the 'prallel' arguments of the function 'IRinfo' into 'TRUE'. Then run the whole code on your R console. This will launch an instance with default 4 CPUs. If your machine is equipped with more cores, you can increase the number of CPUs with the 'nCPU' in 'p.d' subfunction inside the 'IRinfo' function."), br(),
